...
/**
 * 	This class provides an easy way to communicate with the
 * 	doodle.com REST API in order to create, update and vote for polls.
 * 	
 * 	More info can be found at:
 * 	- www.doodle.com/xsd1/RESTfulDoodle.pdf
...
public class DoodlePoll {
	...
	/*
	 * 	Your credentials for using the doodle.com REST API
	 * 	more info at http://doodle.com/xsd1/AAforRESTfulDoodle.pdf
	 */
    
	/**
     * 	Holds your consumer key for doodle oAuth authentification
     */
	private final static String consumerKey          = "ABC";
	
	/**
     * 	Holds your consumer secret for doodle oAuth authentification
     */
	private final static String consumerSecret       = "XYZ";
    ...
    /**
     * 	Indicates if this is a newly generated doodle or an existing one.
     * 	If true, the poll gets posted when calling doodle.save(), if not
     * 	the existing poll gets updated.
     */
    ...
    /**
     * 	Determines if the poll is either to find a meeting date
     * 	or to choose between different text options.
     */
     ...
     * 	Doodle API also allows to connect this with a user id
     * 	instead, but currently this is not supported.
     */
     ...
     * 
     * 	2 = vote can be YES/NO
     * 	3 = vote can be YES/NO/MAYBE
     */
    ...
    /**
     * 	Creates a new/empty doodle poll object and connects
     * 	to the doodle REST API via oAuth
     */
    public DoodlePoll() {
    	isNewDoodle = true;
    ...
    /**
     * 	Creates a new doodle poll object based on an
     * 	existing poll and tries to populate it, by connecting
     * 	to the doodle REST API and requesting the poll data.
     * 
     * 	@param pollId
	...
	/**
	 * 	Indicates if this is a new poll (not saved at doodle)
	 * 	or an existing one.
	 * 
	 * 	@return
	 * 		True if new doodle, false if not.
	 */
	...
	/**
	 * 	Connects to the doodle REST API via oAuth using the
	 * 	provided credentials.
	 * 
	 * 	@return
	 * 		true, if access token was recieved, false if not.
	 */
	private boolean connectOAuth() {
		try {
			/*
	         * Round one, two-legged OAuth to access polls OAuth is just used to authenticate the consumer (you) against our server
	         */
	        consumer = new DefaultOAuthConsumer( consumerKey, consumerSecret, SignatureMethod.HMAC_SHA1 );
	        serviceProvider = new DefaultOAuthProvider( consumer, BASE_URL + requestTokenURL, BASE_URL + accessTokenURL, BASE_URL + userAuthorizationURL );
        
			serviceProvider.retrieveRequestToken( null );
			serviceProvider.retrieveAccessToken( null );
	 ...
	 * 	
	 * 	IMPORTANT: The tokens have a timeout - handling of this is not supported yet.
	 * 
	 * 	@return
	...
	/**
	 * 	When the object should handle an existing doodle poll,
	 * 	this method is called. It connects to the doodle REST API,
	 * 	requests the saved data of the poll and populates the object with it.
	 * 
	 * 	@return
	 * 		True, if population was successful, false if not.
	 */
	private boolean populate() {
		...
		try {
	        ...
	        this.state 			= xmlPoll.getElementsByTagNameNS( namespaceUri, "state" ).item(0).getTextContent().equalsIgnoreCase(STATE_OPEN) ? STATE_OPEN : STATE_CLOSED;
	        this.optionLevels 	= Integer.parseInt(xmlPoll.getElementsByTagNameNS( namespaceUri, "levels" ).item(0).getTextContent() );
	        ...
	        //extract options
	        xmlOptions = ((Element)xmlPoll.getElementsByTagNameNS( namespaceUri, "options" ).item( 0 )).getElementsByTagNameNS( namespaceUri, "option" );
	        ...
	        xmlParticipants = ((Element)xmlPoll
	        		.getElementsByTagNameNS( namespaceUri, "participants" ).item( 0 ))
	        ...
	        for(int i = 0; i < partLen; i++) {
	        	...
	        	p_name = ((Element) xmlSingleParticipant).getElementsByTagNameNS( namespaceUri, "name" ).item(0).getTextContent();
	        	pId = ((Element) xmlSingleParticipant).getElementsByTagNameNS( namespaceUri, "id" ).item(0).getTextContent();
	        	...
	        	xmlSinglePreferences = ((Element) ((Element) xmlSingleParticipant)
	        			.getElementsByTagNameNS( namespaceUri, "preferences" ).item( 0 ))
	...
	/**
	 * 	Tries to find a participant, according to the name
	 * 	he attented to this poll.
	 * 
	 * 	@param pName
	...
	/**
	 * 	Creates a new participant and adds them to the poll.
	 * 	By default, all the preferences of the participant
	 * 	are set to "NO".
	 * 
	 * 	@param pName
	 * 		The name of the participant.
	 * 	@return
	 * 		The new created participant. If a participant with the same
	 * 		name already exists, this one is returned and no new
	...
	public DoodlePollParticipant addParticipant(String pName) {
		...
		for(int i = 0; i < options.size(); i++) {
			...
			DoodlePollPreference pref = new DoodlePollPreference(newParticipant, currOption, DoodlePollPreference.OPTION_NO);
	 ...
	 * 	@return
	 * 		The option.
	 * 		If no option with this value exists, the method returns null.
	 */
	...
	/**
	 * 	Adds a new option with the given value.
	 * 	If an option with this value already exists,
	 * 	no new option is created and the existing option
	 * 	is returned.
	 * 	By default, every existing user votes "NO" for the
	 * 	new option.
	 * 	
	 * 	@param value
	 * 		The value of the option
	 * 	@return
	 * 		The new created option (see desc)
	 */
	public DoodlePollOption addOption(String value) {
		...
		for(int i = 0; i < participants.size(); i++) {
			...
			DoodlePollPreference pref = new DoodlePollPreference(currParticipant, newOption, DoodlePollPreference.OPTION_NO);
	...
	/**
	 * 	Creates the xml representation of the poll for the
	 * 	doodle REST API.
	 * 
	 * 	@return
	 * 		The xml doc for the doodle REST API as a string
	 */
	...
	/**
	 * 	Saves the poll to the doodle REST API.
	 * 	Depending on if this is a new created poll or not,
	...
	/**
	 * 	Converts the xml response of the doodle REST API
	 * 	to a xml document.
	 * 
	 * 	@param xmlStream
	 ...
	 */
	private static Document streamToDocument( InputStream xmlStream )
    throws SAXException,
        IOException,
	...
	/**
	 * 	Saves the secret doodle xKey.
	 * 	The xKey is needed to update existing polls.
	 * 
	 * @param xDoodleKey
	 * 		The xKey to be saved.
	 */
	private void setXKey(String xDoodleKey) {
		//TODO: Implement this method the way you want.
	}
	...
	/**
	 * 	Set the type of this poll.
	 * 	Use constants DoodlePoll.DATE_POLL or DoodlePoll.TEXT_POLL
	 * 
	 * 	@param desc
	 * 		The new type.
	 */
	 ...
	 * 		Either DoodlePoll.DATE_POLL or DoodlePoll.TEXT_POLL
	 */
